...
impl BartEncoderLayer {
    pub fn forward(
        ...
        // Pass the layer_idx down to the attention function for conditional logging
        ...
        // Print the final output of Layer 0 to confirm the input to Layer 1 is correct.
        if layer_idx == 1 {
            println!("\n--- RUST ENCODER LAYER 0: Final Output (Verified Input to Layer 1) ---");
            println!("Shape: {:?}", final_hidden_states.dim());
            println!(
                "First vector (first 5 values): {:?}",
                final_hidden_states.slice(s![0, 0, ..5])
            );
            println!("{}", "-".repeat(20));
        }
...
impl BartEncoder {
    pub fn forward(
        ...
        for (i, layer) in self.layers.iter().enumerate() {
            hidden_states = layer.forward(&hidden_states, attention_mask, i)?;
        }
        Ok(hidden_states)
    }
}
    ...
    pub fn from_weights(
        ...
        for i in 0..config.encoder_layers {
            ...
            if i == 1 {
                let prefix = format!("model.encoder.layers.{}", i);
                let k_proj_weight =
                    weights.get_array2(&format!("{}.self_attn.k_proj.weight", prefix))?;
                let k_proj_bias =
                    weights.get_array1(&format!("{}.self_attn.k_proj.bias", prefix))?;

                println!("\n--- RUST LAYER 1 WEIGHTS ---");
                // Print the first 5 values from the first row of the K-projection weight matrix
                println!(
                    "K-Proj Weight [Row 0, Vals 0-4]: {:?}",
                    k_proj_weight.slice(s![0, ..5])
                );
                // Print the first 5 values of the K-projection bias vector
                println!("K-Proj Bias [Vals 0-4]: {:?}", k_proj_bias.slice(s![..5]));
            }
